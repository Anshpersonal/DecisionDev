...
from app.service.decision_service import DecisionServiceFactory
import os
...
decision_services = DecisionServiceFactory.get_available_services()
tools = DecisionServiceFactory.load_tools()
workflow_service = WorkflowService(llm, rag_service, decision_services, tools)

# Initialize LangSmith client if API key is available
langsmith_client = Client() if config.LANGCHAIN_API_KEY else None
...
def health_check():
    ...
    status = {
        "status": "healthy",
        "llm": llm_service.llm.__class__.__name__,
        "decision_services": list(decision_services.keys()),
        "tools_count": len(tools),
        "rag_initialized": rag_service.chain is not None,
